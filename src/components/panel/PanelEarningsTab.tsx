...
export default function PanelEarningsTab() {
  ...
  const handleClaim = async (tokenId: string) => {
    ...
    try {
      ...
      toast({ title: "Fees claimed!", description: `You claimed ${formatSolAmount(result.claimedAmount)} SOL` });
      refetch();
    } catch (error) {
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 gap-2">
          <Card className="p-3 bg-white/5 border-white/10">
            ...
            <p className="text-lg font-bold" style={{ color: "#4ade80" }}>{formatSolAmount(earningsData?.summary?.totalEarned || 0)} SOL</p>
          </Card>
          <Card className="p-3 bg-white/5 border-white/10">
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground mb-1">
              <DollarSign className="h-3.5 w-3.5" />Unclaimed
            </div>
            <p className="text-lg font-bold text-green-500">{formatSolAmount(earningsData?.summary?.totalUnclaimed || 0)} SOL</p>
          </Card>
        </div>
      )}
          ...
          {earningsData.claims.slice(0, 5).map((claim: any) => (
            <Card key={claim.id} className="p-3 flex items-center gap-3 bg-white/[0.02] border-white/10">
              ...
              <div className="flex-1">
                <p className="font-medium text-sm">{formatSolAmount(claim.amount_sol)} SOL</p>
                <p className="text-xs text-muted-foreground">{new Date(claim.created_at).toLocaleDateString()}</p>
              </div>
              <a href={`https://solscan.io/tx/${claim.signature}`} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline">View Tx</a>
            </Card>
          ))}
        </div>
      )}
