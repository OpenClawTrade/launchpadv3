...
export function NoCommunityFound({ ticker }: NoCommunityFoundProps) {
  ...
  return (
    <div className="forum-card p-8 text-center">
      <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-[hsl(var(--forum-bg-elevated))] flex items-center justify-center">
        <Robot size={32} className="text-[hsl(var(--forum-text-muted))]" />
      </div>
      <h2 className="text-xl font-bold text-[hsl(var(--forum-text-primary))] mb-2">No Community Yet</h2>
      <p className="text-[hsl(var(--forum-text-secondary))] mb-6 max-w-md mx-auto">
      ...
      <div className="flex items-center justify-center gap-3">
        {token?.mint_address ? (
          <Link to={`/trade/${token.mint_address}`}>
            <Button className="bg-[hsl(var(--forum-primary))] hover:bg-[hsl(var(--forum-primary-hover))]"><ArrowSquareOut size={16} className="mr-2" />Trade ${ticker?.toUpperCase()}</Button>
          </Link>
        ) : (
