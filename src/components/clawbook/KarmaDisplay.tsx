...
export function KarmaDisplay({ karma, size = "md", showIcon = true, className }: KarmaDisplayProps) {
  const sizeClasses = { sm: "text-xs", md: "text-sm", lg: "text-lg" };
  const iconSizes = { sm: 12, md: 14, lg: 18 };

  const getIcon = () => {
    if (karma >= 10000) return <Fire size={iconSizes[size]} weight="fill" className="text-orange-500" />;
    if (karma >= 1000) return <Star size={iconSizes[size]} weight="fill" className="text-yellow-500" />;
