...
function parseBlocks(content: string): Block[] {
  ...
  const flushParagraph = () => { if (currentParagraph.length > 0) { blocks.push({ type: "paragraph", lines: currentParagraph }); currentParagraph = []; } };
  const flushList = () => { if (currentList.length > 0) { blocks.push({ type: "list", items: currentList }); currentList = []; } };

  for (const line of lines) {
    const trimmed = line.trim();
    if (/^-{3,}$/.test(trimmed)) { flushParagraph(); flushList(); blocks.push({ type: "hr" }); continue; }
    if (trimmed.startsWith("## ")) { flushParagraph(); flushList(); blocks.push({ type: "header", level: 2, text: trimmed.slice(3) }); continue; }
    if (trimmed.startsWith("### ")) { flushParagraph(); flushList(); blocks.push({ type: "header", level: 3, text: trimmed.slice(4) }); continue; }
    if (trimmed.startsWith("- ")) { flushParagraph(); currentList.push(trimmed.slice(2)); continue; }
...
function formatLine(text: string): React.ReactNode[] {
  ...
  while (remaining.length > 0) {
    ...
    if (boldMatch) { elements.push(<strong key={key++} className="font-semibold">{boldMatch[1]}</strong>); remaining = remaining.slice(boldMatch[0].length); continue; }
    const italicMatch = remaining.match(/^\*([^*]+)\*/);
    if (italicMatch && !remaining.match(/^\*https?:\/\//)) { elements.push(<em key={key++} className="italic">{italicMatch[1]}</em>); remaining = remaining.slice(italicMatch[0].length); continue; }
    ...
    if (nextSpecial === -1) { elements.push(<Fragment key={key++}>{remaining}</Fragment>); break; }
    else if (nextSpecial === 0) { elements.push(<Fragment key={key++}>{remaining[0]}</Fragment>); remaining = remaining.slice(1); }
    else { elements.push(<Fragment key={key++}>{remaining.slice(0, nextSpecial)}</Fragment>); remaining = remaining.slice(nextSpecial); }
  }
  return elements;
}
...
function truncateUrl(url: string, maxLength = 40): string {
  ...
  try {
    ...
    return domain + path;
  } catch { return url.slice(0, maxLength - 3) + "..."; }
}