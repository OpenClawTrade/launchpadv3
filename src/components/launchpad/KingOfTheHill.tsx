...
import { Users, Bot, BadgeCheck, TrendingUp, BarChart3, ArrowUpRight, Globe, MessageCircle, Copy, Check, Zap, Crown, Pencil } from "lucide-react";
...
/* ── premium progress bar — thicker on mobile ── */
function ProgressBar({ value, mobile }: { value: number; mobile?: boolean }) {
  const [w, setW] = useState(0);
  useEffect(() => { const t = setTimeout(() => setW(Math.min(value, 100)), 200); return () => clearTimeout(t); }, [value]);

  const barH = mobile ? "h-[8px]" : "h-[6px]";

  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-1.5">
        <span
          className="uppercase tracking-[0.15em] font-semibold font-mono"
          style={{ fontSize: mobile ? "11px" : "9px", color: "#6E6E80" }}
        >
          Bonding Progress
        </span>
        <span
          className="font-bold font-mono tabular-nums"
          style={{ fontSize: mobile ? "13px" : "12px", color: "#FFFFFF" }}
        >
          {value.toFixed(0)}%
        </span>
      </div>
      <div className={cn("relative w-full rounded-full overflow-hidden", barH)} style={{ background: "rgba(255,255,255,0.06)" }}>
        <div
...
/* ═══════════════════════════════════════════════════════════
   MOBILE CARD — full-width, vertical stacked, touch-first
   ═══════════════════════════════════════════════════════════ */
function MobileKingCard({ token, rank, quickBuyAmount, sparklineData }: { token: KingToken; rank: number; quickBuyAmount: number; sparklineData?: number[] }) {
...
/* ═══════════════════════════════════════════════════════════
   DESKTOP CARD — same as before with minor refinements
   ═══════════════════════════════════════════════════════════ */
function DesktopKingCard({ token, rank, quickBuyAmount, sparklineData }: { token: KingToken; rank: number; quickBuyAmount: number; sparklineData?: number[] }) {
  const navigate = useNavigate();
  const funToken = useMemo(() => kingToFunToken(token), [token]);
  const [copied, setCopied] = useState(false);
  ...
  return (
    <div
      ...
      className="group relative flex flex-col cursor-pointer flex-1"
      style={{
    ...
    >
      {r.king && (
        <div className="absolute -top-px left-4 right-4 h-[1px] rounded-full" style={{ background: "linear-gradient(90deg, transparent, rgba(245,158,11,0.5), rgba(255,215,0,0.6), rgba(245,158,11,0.5), transparent)" }} />
      )}
      ...
      {/* Cosmic specks */}
      <div className="absolute inset-0 pointer-events-none rounded-[20px] overflow-hidden" style={{ opacity: 0.4 }}>
        <div className="absolute w-1 h-1 rounded-full" style={{ background: "rgba(0,212,255,0.3)", top: "15%", left: "80%", filter: "blur(1px)" }} />
        <div className="absolute w-0.5 h-0.5 rounded-full" style={{ background: "rgba(0,255,170,0.2)", top: "60%", left: "20%", filter: "blur(0.5px)" }} />
      </div>
      ...
      <div className="relative z-10 flex items-center gap-3 mb-3">
        <div
          ...
          style={{ background: r.badgeBg, boxShadow: r.badgeShadow, width: r.king ? "40px" : "34px", height: r.king ? "40px" : "34px", fontSize: r.king ? "15px" : "13px", animation: r.crownPulse ? "koth-crown-pulse 2s ease-in-out infinite" : undefined }}
        >
          {r.label}
          {r.king && <Crown className="absolute -top-2 -right-1.5" style={{ width: "14px", height: "14px", color: "#FFD700", filter: "drop-shadow(0 0 4px rgba(255,215,0,0.5))" }} />}
        </div>
        <div className="flex-shrink-0 overflow-hidden" style={{ width: r.king ? "48px" : "42px", height: r.king ? "48px" : "42px", borderRadius: "14px", border: "1px solid rgba(255,255,255,0.06)", boxShadow: isHovered ? "0 0 12px rgba(0,212,255,0.15)" : "none" }}>
          <OptimizedTokenImage src={token.image_url} alt={token.name} fallbackText={token.ticker} size={96} className="w-full h-full object-cover" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 flex-wrap overflow-visible">
            <span className="font-bold leading-tight truncate max-w-[110px]" style={{ color: "#FFFFFF", fontSize: r.king ? "16px" : "14px", letterSpacing: "0.2px" }}>{token.name}</span>
            <span className="font-mono flex-shrink-0 whitespace-nowrap" style={{ color: "#6E6E80", fontSize: "11px" }}>${token.ticker}</span>
            <LiveAge createdAt={token.created_at} className="text-[9px]" />
            {isTrader && (
              <span className="text-[8px] font-semibold px-1.5 py-0.5 rounded-md uppercase tracking-wider flex-shrink-0" style={{ background: "rgba(0,212,255,0.08)", color: "#00D4FF", border: "1px solid rgba(0,212,255,0.12)" }}>
          ...
          <div className="flex items-center gap-1 mt-0.5 min-h-[16px]">
            <svg className="w-3 h-3 flex-shrink-0" style={{ color: "#4A4A5A" }} viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
            {xUser ? (
              <>
                {xAvatar && <img src={xAvatar} alt="" className="w-3.5 h-3.5 rounded-full object-cover flex-shrink-0" style={{ border: "1px solid rgba(255,255,255,0.08)" }} />}
      ...
      <div className="relative mb-3 rounded-xl overflow-hidden" style={{ minHeight: "72px" }}>
        ...
        <div className="relative z-10 flex flex-col gap-2 p-2">
          <div>
            <span className="block mb-0.5 uppercase tracking-[0.12em] font-mono font-semibold" style={{ fontSize: "9px", color: "#6E6E80" }}>MCap</span>
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="font-black font-mono tabular-nums leading-none" style={{ fontSize: "17px", color: "#FFD700", textShadow: "0 0 12px rgba(255,215,0,0.2)" }}>
                ${mcapUsd >= 1_000_000 ? `${(mcapUsd / 1_000_000).toFixed(2)}M` : mcapUsd >= 1_000 ? `${(mcapUsd / 1_000).toFixed(1)}K` : mcapUsd.toFixed(0)}
              </span>
              {change24h !== 0 && (
                <span className="font-mono font-bold px-1.5 py-0.5 rounded-md" style={{ fontSize: "10px", color: change24h > 0 ? "#00FFAA" : "#FF4D4D", background: change24h > 0 ? "rgba(0,255,170,0.1)" : "rgba(255,77,77,0.1)", border: `1px solid ${change24h > 0 ? "rgba(0,255,170,0.15)" : "rgba(255,77,77,0.15)"}` }}>
                  {formatChange24h(change24h)}
                </span>
              )}
            </div>
          ...
          <div>
            <span className="uppercase tracking-[0.12em] font-mono font-semibold" style={{ fontSize: "9px", color: "#6E6E80" }}>Vol 24h</span>
      ...
      {/* Progress */}
      <div className="relative z-10 mb-3"><ProgressBar value={progress} /></div>
        ...
        <TooltipProvider delayDuration={200}>
          {token.twitter_url && (
            <Tooltip><TooltipTrigger asChild>
              <button onClick={(e) => handleSocialClick(e, token.twitter_url!)} className="p-1.5 rounded-lg transition-all duration-300" style={{ color: "#6E6E80" }}
            ...
            </TooltipTrigger><TooltipContent side="bottom" className="text-[10px]">Twitter</TooltipContent></Tooltip>
          )}
          {token.telegram_url && (
            <Tooltip><TooltipTrigger asChild>
              <button onClick={(e) => handleSocialClick(e, token.telegram_url!)} className="p-1.5 rounded-lg transition-all duration-300" style={{ color: "#6E6E80" }}
                onMouseEnter={e => { (e.target as HTMLElement).style.color = "#00D4FF"; (e.target as HTMLElement).style.background = "rgba(0,212,255,0.08)"; }}
                onMouseLeave={e => { (e.target as HTMLElement).style.color = "#6E6E80"; (e.target as HTMLElement).style.background = "transparent"; }}>
                <MessageCircle className="w-3 h-3" />
              </button>
            </TooltipTrigger><TooltipContent side="bottom" className="text-[10px]">Telegram</TooltipContent></Tooltip>
          )}
          {token.website_url && (
            <Tooltip><TooltipTrigger asChild>
              <button onClick={(e) => handleSocialClick(e, token.website_url!)} className="p-1.5 rounded-lg transition-all duration-300" style={{ color: "#6E6E80" }}
                onMouseEnter={e => { (e.target as HTMLElement).style.color = "#00D4FF"; (e.target as HTMLElement).style.background = "rgba(0,212,255,0.08)"; }}
                onMouseLeave={e => { (e.target as HTMLElement).style.color = "#6E6E80"; (e.target as HTMLElement).style.background = "transparent"; }}>
                <Globe className="w-3 h-3" />
              </button>
            </TooltipTrigger><TooltipContent side="bottom" className="text-[10px]">Website</TooltipContent></Tooltip>
          )}
          {token.mint_address && (
            <Tooltip><TooltipTrigger asChild>
              <button onClick={handleCopyCA} className="p-1.5 rounded-lg transition-all duration-300" style={{ color: copied ? "#00FFAA" : "#6E6E80", background: copied ? "rgba(0,255,170,0.08)" : "transparent" }}>
                {copied ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
              </button>
            </TooltipTrigger><TooltipContent side="bottom" className="text-[10px]">{copied ? "Copied!" : "Copy CA"}</TooltipContent></Tooltip>
          )}
          {codexChartUrl && (
            <Tooltip><TooltipTrigger asChild>
              <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); window.open(codexChartUrl, "_blank"); }} className="p-1.5 rounded-lg transition-all duration-300" style={{ color: "#6E6E80" }}
                onMouseEnter={e => { (e.target as HTMLElement).style.color = "#00D4FF"; (e.target as HTMLElement).style.background = "rgba(0,212,255,0.08)"; }}
                onMouseLeave={e => { (e.target as HTMLElement).style.color = "#6E6E80"; (e.target as HTMLElement).style.background = "transparent"; }}>
                <BarChart3 className="w-3 h-3" />
              </button>
            </TooltipTrigger><TooltipContent side="bottom" className="text-[10px]">Chart</TooltipContent></Tooltip>
          )}
        </TooltipProvider>
      </div>

      {/* Actions */}
      <div className="relative z-10 flex items-center gap-2" onClickCapture={e => e.stopPropagation()} onMouseDownCapture={e => e.stopPropagation()} onTouchStartCapture={e => e.stopPropagation()} onPointerDownCapture={e => e.stopPropagation()}>
        <button
          onClick={(e) => { e.stopPropagation(); navigate(url); }}
          ...
          style={{ height: "34px", borderRadius: "12px", fontSize: "12px", color: "#00D4FF", background: "rgba(0,212,255,0.06)", border: "1px solid rgba(0,212,255,0.15)", letterSpacing: "0.3px" }}
          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = "rgba(0,212,255,0.12)"; (e.currentTarget as HTMLElement).style.borderColor = "rgba(0,212,255,0.3)"; }}
          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = "rgba(0,212,255,0.06)"; (e.currentTarget as HTMLElement).style.borderColor = "rgba(0,212,255,0.15)"; }}
        >
          <TrendingUp style={{ width: "13px", height: "13px" }} />Trade
        </button>
...
function KingCardSkeleton({ mobile }: { mobile?: boolean }) {
  ...
  return (
    <div className="flex flex-col rounded-[20px]" style={{ background: "linear-gradient(165deg, rgba(15,15,26,0.95) 0%, rgba(5,5,15,0.98) 100%)", border: "1px solid rgba(255,255,255,0.04)", padding: "16px", flex: 1 }}>
      <div className="flex items-center gap-3 mb-3"><Skeleton className="w-10 h-10 rounded-xl" /><Skeleton className="w-12 h-12 rounded-[14px]" /><div className="flex-1 space-y-1.5"><Skeleton className="h-4 w-24" /><Skeleton className="h-3 w-16" /></div></div>
      <div className="space-y-2 mb-3"><Skeleton className="h-[72px] w-full rounded-xl" /></div>
      <Skeleton className="h-[6px] w-full rounded-full mb-3" />
      <div className="flex items-center gap-2 pt-2"><Skeleton className="h-[34px] flex-1 rounded-xl" /><Skeleton className="h-[34px] flex-1 rounded-xl" /></div>
    </div>
  );
}

/* ═══════════════════════════════════════════════════════════
   MAIN EXPORT — responsive: vertical stack on mobile, row on desktop
   ═══════════════════════════════════════════════════════════ */
export function KingOfTheHill() {
  const { tokens, isLoading } = useKingOfTheHill();
  const [quickBuyAmount, setQuickBuyAmount] = useState(() => {
    try { const v = localStorage.getItem("pulse-quick-buy-amount"); if (v) { const n = parseFloat(v); if (n > 0 && isFinite(n)) return n; } } catch {} return 0.5;
  });
  ...
  const commitQuickBuy = () => {
    ...
    if (n > 0 && isFinite(n)) { setQuickBuyAmount(n); try { localStorage.setItem("pulse-quick-buy-amount", String(n)); } catch {} }
    else { setQuickBuyInput(String(quickBuyAmount)); }
    setEditingQuickBuy(false);
  };

  useEffect(() => { if (editingQuickBuy) quickBuyInputRef.current?.focus(); }, [editingQuickBuy]);

  const sparklineAddresses = useMemo(() => (tokens ?? []).map(t => t.mint_address).filter(Boolean) as string[], [tokens]);
