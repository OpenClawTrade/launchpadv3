  ...
  const MobileTokenCard = ({ token, index }: { token: Token; index: number }) => {
    ...
    return (
      <Link 
        ...
        className="block p-3 border-b border-border last:border-b-0 hover:bg-secondary/30 transition-colors"
      >
        <div className="flex items-start gap-3">
          {/* Avatar */}
          <div className="gate-token-avatar flex-shrink-0">
            {token.image_url ? (
          ...
          {/* Right Side: Price Change & Market Cap */}
    ...
    <Card className="gate-card">
        ...
        <div className="divide-y divide-border">
          {isLoading ? (
            ...
            paginatedTokens.map((token, index) => (
              <MobileTokenCard key={token.id} token={token} index={(page - 1) * pageSize + index} />
            ))
          )}
        </div>
      ) : (
      ...
      {totalPages > 1 && (
        <div className="gate-pagination flex-wrap gap-2">
          <span className="gate-pagination-info text-xs sm:text-sm">
