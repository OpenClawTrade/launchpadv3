    ...
    <section className="py-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 mb-8">
        {isLoading ? (
          ...
          <p className="col-span-full text-center py-8" style={{ color: "hsl(var(--claw-muted))" }}>
          ...
          agents?.map((agent) => {
            ...
            return (
              <button
                ...
                className="claw-card p-3 text-left transition-all cursor-pointer"
                style={{
              ...
              >
                ...
                <p className="text-xs font-mono" style={{ color: "hsl(var(--claw-muted))" }}>
                  ${agent.ticker}
                </p>
              </button>
            );
        ...
        <div className="claw-card p-6 max-w-md mx-auto">
          ...
          {!initResult ? (
            <div className="space-y-4">
              <div>
                <label className="text-xs font-semibold uppercase tracking-wider block mb-2" style={{ color: "hsl(var(--claw-muted))" }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    border: "1px solid hsl(var(--claw-border))",
              ...
              <button
                ...
                className="w-full py-3 rounded-lg font-bold text-sm uppercase tracking-wider transition-all disabled:opacity-50 claw-bribe-btn"
              >
                {isInitializing ? (
              ...
              <div className="relative">
                ...
                <button onClick={copyAddress} className="absolute right-2 top-1/2 -translate-y-1/2">
                  {copied ? (
                    ...
                    <Copy className="w-4 h-4" style={{ color: "hsl(var(--claw-muted))" }} />
                  )}
                </button>
              </div>
      ...
      {confirmResult?.success && confirmResult.childAgent && (
        <div className="claw-card p-6 max-w-md mx-auto text-center mt-6" style={{ borderColor: "hsl(142 71% 45% / 0.5)" }}>
          <Sparkles className="w-8 h-8 mx-auto mb-3" style={{ color: "hsl(45 93% 58%)" }} />
          <h3 className="text-lg font-black mb-2 claw-gradient-text">New Agent Spawned!</h3>
          <div className="flex items-center justify-center gap-3 mb-3">
            ...
            <div>
              <p className="font-bold" style={{ color: "hsl(var(--claw-text))" }}>
                {confirmResult.childAgent.name}
              </p>
              <p className="text-sm font-mono" style={{ color: "hsl(var(--claw-muted))" }}>
                ${confirmResult.childAgent.ticker}
              </p>
            </div>
          </div>
          <p className="text-sm mb-4" style={{ color: "hsl(var(--claw-muted))" }}>
            {confirmResult.childAgent.description}
          </p>
          <button
            onClick={() => { reset(); setSelectedAgentId(null); setTxSignature(""); }}
            className="text-sm font-semibold underline"
            style={{ color: "hsl(var(--claw-secondary))" }}
          >
            Bribe Another Agent
          </button>
        </div>
