...
import { Radar, RefreshCw, AlertTriangle, Clock, Activity } from "lucide-react";
            ...
            {latestRun.errors_count > 0 && errors.length > 0 && (
              <div className="mt-2 p-2 rounded-lg bg-red-500/5 border border-red-500/20">
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[10px] font-bold text-red-400">{latestRun.errors_count} errors</span>
                </div>
                <div className="space-y-1 max-h-32 overflow-y-auto">
                  {errors.slice(0, 5).map((e) => (
                    <div key={e.id} className="text-[10px] text-muted-foreground">
                      <span className="font-mono text-red-300">@{e.kol_username}</span>: {e.error_message.substring(0, 150)}
                    </div>
                  ))}
                </div>
              </div>
            )}
            {latestRun.raw_response_sample && (
              <details className="mt-2">
                <summary className="text-[10px] text-muted-foreground cursor-pointer hover:text-foreground">
                  Response structure sample
                </summary>
                <pre className="mt-1 text-[9px] text-muted-foreground bg-secondary/20 p-2 rounded overflow-x-auto max-h-24">
                  {latestRun.raw_response_sample}
                </pre>
              </details>
            )}
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              The scanner runs every 5 minutes. Use "Run Scan" to trigger manually.
            </p>
          </div>
        )}
      </div>
