// DISABLED — @clawmode mention launcher fully suspended
...
serve(async (req) => {
  ...
  return new Response(JSON.stringify({ success: true, message: "Mention launcher is disabled" }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
});
...
Launch your unique Solana Agent from Claw Mode`;
    ...
    const websiteUrl = `https://clawsai.fun/t/${tokenConcept.ticker.toUpperCase()}`;
