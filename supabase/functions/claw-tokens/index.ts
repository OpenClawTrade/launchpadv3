...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Query claw_agent_tokens joined with claw_tokens and claw_agents
    let query = supabase
    ...
    if (error) {
      throw new Error(`Failed to fetch claw tokens: ${error.message}`);
    }

    // Transform and sort
    let tokens = (agentTokens || []).map((at: any) => {
      const token = at.claw_tokens;
      const agent = at.claw_agents;
      return {
        id: at.id,
        agentId: at.agent_id,
        ...
        token: token ? {
          ...
          launchpadType: token.launchpad_type || 'claw',
        } : null,
      };
    }).filter((t: any) => t.token !== null);

    // Client-side sorting
    if (sort === "mcap") {
      tokens = tokens.sort((a: any, b: any) => (b.token?.marketCapSol || 0) - (a.token?.marketCapSol || 0));
    } else if (sort === "volume") {
      tokens = tokens.sort((a: any, b: any) => (b.token?.volume24hSol || 0) - (a.token?.volume24hSol || 0));
    } else if (sort === "hot") {
    ...
    console.error("claw-tokens error:", error);
    return new Response(
