 ...
 * 
 * Users who launch tokens via Twitter (!tunalaunch) don't get an API key immediately.
  ...
  try {
    ...
    if (!agent) {
      return new Response(
        JSON.stringify({ 
          success: false, 
          error: "No agent found for this wallet. Launch a token first via Twitter @TunaLaunch." 
        }),
